/* @(#)k_rem_pio2.c 1.3 95/01/18 -- SunSoft, see include/CWalkyMath.h
 *
 * The Payne-Hanek reduction, for |x| >= 2**20. This model cannot reach it:
 * codec.ts:138 caps a coordinate at 2**22 and the wander at behaviour.ts:945
 * scales by 0.031 and 0.017, so the largest argument sin() ever sees is about
 * 201,000 -- five times inside the medium branch. It is ported in full anyway,
 * because a routine that is correct up to a threshold and silently wrong past
 * it is the kind of thing that is discovered years later by a saved map.
 */
#include "fdlibm.h"
#include <math.h>
...
recompute:
  ...
  if (ih > 0) {
    ...
    if (ih == 2) { z = one - z; if (carry != 0) z -= scalbn(one, q0); }
  }
...
int walky_kernel_rem_pio2(double *x, double *y, int e0, int nx, int prec, const int32_t *ipio2) {
  return kernel_rem_pio2(x, y, e0, nx, prec, ipio2);
}
