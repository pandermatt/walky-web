/* @(#)e_pow.c 1.5 04/04/22 -- SunSoft, see include/CWalkyMath.h */
#include "fdlibm.h"
...
double walky_pow(double x, double y) {
  ...
  if ((iy | ly) == 0) return one;                       /* y == 0 */
  if (hx == 0x3ff00000 && lx == 0) return one;          /* x == 1 */
  if (ix > 0x7ff00000 || ((ix == 0x7ff00000) && (lx != 0)) ||
      iy > 0x7ff00000 || ((iy == 0x7ff00000) && (ly != 0)))
    return (x + 0.0) + (y + 0.0);                       /* NaN */

  /* Is y an odd int, an even int, or not an int? */
  yisint = 0;
  if (hx < 0) {
  ...
  if (ly == 0) {
    if (iy == 0x7ff00000) {                             /* y is +-inf */
      if (((ix - 0x3ff00000) | lx) == 0) return one;    /* (-1)**+-inf is 1 */
      if (ix >= 0x3ff00000) return (hy >= 0) ? y : zero;
      return (hy < 0) ? -y : zero;
    }
    if (iy == 0x3ff00000) return (hy < 0) ? one / x : x;
    if (hy == 0x40000000) return x * x;
    if (hy == 0x3fe00000) { if (hx >= 0) return sqrt(x); }
  }
  ...
  if (lx == 0) {
    if (ix == 0x7ff00000 || ix == 0 || ix == 0x3ff00000) {   /* x is +-0, +-inf, +-1 */
      ...
      if (hy < 0) z = one / z;
      if (hx < 0) {
        if (((ix - 0x3ff00000) | yisint) == 0) z = (z - z) / (z - z);  /* (-1)**non-int */
  ...
  n = (hx >> 31) + 1;
  if ((n | yisint) == 0) return (x - x) / (x - x);      /* (-x)**non-int is NaN */

  s = one;
  if ((n | (yisint - 1)) == 0) s = -one;                /* (-x)**odd is -(x**odd) */
