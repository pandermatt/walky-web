/* @(#)e_rem_pio2.c 1.4 95/01/18 -- SunSoft, see include/CWalkyMath.h */
#include "fdlibm.h"
#include <math.h>

/* Bits of 2/pi, 24 at a time. */
...
int walky_rem_pio2(double x, double *y) {
  ...
  /* |x| >= 2**19 pi/2: the Payne-Hanek path. Unreachable in this model. */
  GET_HIGH_WORD(hx, x);
  e0 = (ix >> 20) - 1046;                                    /* e0 = ilogb(x) - 23 */
  INSERT_WORDS(z, ix - ((int32_t)(e0 << 20)), 0);
  GET_LOW_WORD(low, x);
  SET_LOW_WORD(z, low);
  ...
  n = walky_kernel_rem_pio2(tx, y, e0, nx, 2, two_over_pi);
  if (hx < 0) { y[0] = -y[0]; y[1] = -y[1]; return -n; }
  return n;
}
