 ...
 * than copied and a snapshot costs a handful of objects. Trees and the crowd
 * are the same story -- see Agents.snapshot for what the crowd keeps.
 */
...
export class App {
  ...
  private context: ToolContext = {
    ...
    addTree: (at) => {
      this.checkpoint();
      this.trees = [...this.trees, makeTree(at)];
      this.touch();
    },
  ...
  private async clearMap(): Promise<void> {
    ...
    if (!empty) {
      const confirmed = await confirmAction({
        title: 'Clear the map?',
        message: 'Every wall, tree and pedestrian goes. Undo brings them back.',
        confirmLabel: 'Clear',
        destructive: true,
      });
      if (!confirmed) return;
